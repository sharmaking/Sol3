...
public:
	...
	virtual void OnError(std::string error){};	//错误信息
	GetDataSpi(){};
	~GetDataSpi(){};
};
...
private:
	bool nAutoRecon;		//自动重连
	int nReconTimeOut;		//重连时间间隔
...
public:
	...
	//请求实时行情
	void ReqRealtimeData(std::vector<char*> &subCodes, bool isAllMarket, GD_ErrorType &err);
	void ReqUpdateSubStockCode(GD_UpdateSubCodeType reqType, std::vector<char*> &subCodes, bool isAllMarket, GD_ErrorType &err);		//订阅股票 vec为空代表订阅所有
	//请求模拟行情
	void ReqSimulationData(GD_ISODateTimeType beginTime, GD_ISODateTimeType endTime, GD_SimuModelType modelType, std::vector<char*> &subCodes, bool isAllMarket, GD_ErrorType &err);	//请求模拟行情
	void ReqGDKLine(GD_CycType type, GD_ISODateTimeType beginTime, GD_ISODateTimeType endTime, std::vector<char*> &subCodes, bool isAllMarket, GD_ErrorType &err);	//请求模拟行情	
	//请求TDB历史数据
	...
	GetDataApi(GetDataSpi *spi, bool autoRecon, int reconTimeOut);
	~GetDataApi();	
};

extern "C"
{
	GetData_API GetDataApi* CreateGetDataApi(GetDataSpi* spi, bool autoRecon, int reconTimeOut);
	GetData_API void DelGetDataApi(GetDataApi* pAPI);
}
